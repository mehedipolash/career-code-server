...
app.use(
  cors({
    origin: ["http://localhost:5173"], // update after client deploys
    credentials: true,
  })
);
...
async function run() {
  try {
    ...
    // get all jobs or specific user's jobs by email query
    app.get("/jobs", async (req, res) => {
      const email = req.query.email;
      const query = {};
      if (email) {
    ...
    // get jobs with application count for each job posted by recruiter
    app.get("/jobs/applications", verifyToken, async (req, res) => {
      const email = req.query.email;
      const query = { hr_email: email };
      const jobs = await jobCollection.find(query).toArray();
    ...
    // get all applications or specific user's applications by email query
